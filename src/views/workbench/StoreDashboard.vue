...
async function loadAll() {
  ...
  try {
    const [s, sa, d] = await Promise.all([
  ...
  <PageWrapper title="门店工作台" :subtitle="`欢迎，${user?.realName || '门店'}`" hero>
    ...
    <section class="grid grid-cols-1 md:grid-cols-3 gap-4 animate-fade-in-up">
      ...
      <Card class="hover:shadow-md transition-shadow cursor-pointer" @click="go(ROUTE_PATHS.STORE_SALES)">
        <CardContent class="p-5">
          <div class="text-xs text-muted-foreground mb-2">累计销售额</div>
          <div class="text-2xl font-semibold tabular-nums">{{ formatCurrency(sales.totalAmount) }}</div>
        </CardContent>
      </Card>
      <Card class="hover:shadow-md transition-shadow cursor-pointer" @click="go(ROUTE_PATHS.STORE_SALES)">
        <CardContent class="p-5">
          <div class="text-xs text-muted-foreground mb-2">已上报销量 / 上报天数</div>
          <div class="text-2xl font-semibold tabular-nums">
            {{ formatNumber(sales.totalQty) }}
            <span class="text-sm text-muted-foreground font-normal">/ {{ sales.days }} 天</span>
          </div>
        </CardContent>
      </Card>
    </section>
