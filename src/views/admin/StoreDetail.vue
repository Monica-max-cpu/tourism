...
const item = ref<StoreApply | null>(null)
...
async function load() {
  ...
  try {
    ...
    item.value = await getStoreApplyApi(id)
  } finally {
    loading.value = false
  }
}
...
async function approve() {
  ...
  try {
    await approveStoreApplyApi(item.value.id)
    await load()
...
async function confirmReject() {
  ...
  submitting.value = true
  try {
    await rejectStoreApplyApi(item.value.id, rejectReason.value.trim())
    rejectModal.close()
    await load()
  } finally { submitting.value = false }
}
...
async function confirmToggle() {
  ...
  try {
    ...
    await toggleStoreStatusApi(item.value.id, target)
    toggleModal.close()
    await load()
  } finally { submitting.value = false }
}
...
<template>
  <PageWrapper :title="item ? `门店入驻详情 - ${item.storeName}` : '门店入驻详情'" subtitle="">
    <template #extra>
    ...
    <div v-else-if="!item" class="flex items-center justify-center py-32">
      <span class="text-sm text-muted-foreground">门店不存在</span>
    </div>
          ...
          <div class="bg-white rounded-[16px] overflow-hidden" style="box-shadow: 0 8px 32px hsl(var(--primary) / 0.05);">
            ...
            <div class="p-8 space-y-10">
              ...
              <div class="flex flex-wrap items-center justify-between gap-4">
                <h2 class="text-xl font-bold text-primary">{{ item.storeName }}</h2>
              ...
              <section>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-[15px]">
                  <div><span class="text-muted-foreground">负责人：</span><span class="font-medium">{{ item.contactPerson }}</span></div>
                  <div><span class="text-muted-foreground">电话：</span><span class="font-medium">{{ item.contactPhone }}</span></div>
                  <div><span class="text-muted-foreground">邮箱：</span><span class="font-medium">{{ item.contactEmail || '-' }}</span></div>
                </div>
              </section>
