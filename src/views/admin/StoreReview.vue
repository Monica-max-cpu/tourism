...
import {
  ...
  isPendingStatus, isApprovedStatus, isDisabledStatus,
} from '/@/constants/b2bStatus';
import { formatDateTime } from '/@/utils/format';
import type { StoreApply, ApplyStatus } from '/#/b2b';
...
<template>
  <PageWrapper title="门店入驻审核" subtitle="审核门店提交的入驻申请" variant="hero">
      ...
      <template #action="{ row }">
        <TableAction
          :actions="[
            ...
            { label: '通过', authCode: 'b2b:store:review', hidden: !isPendingStatus(row.status), onClick: () => approve(row) },
            { label: '驳回', authCode: 'b2b:store:review', hidden: !isPendingStatus(row.status), onClick: () => openReject(row) },
            { label: '停用', authCode: 'b2b:store:review', hidden: !isApprovedStatus(row.status), onClick: () => openToggle(row) },
            { label: '启用', authCode: 'b2b:store:review', hidden: !isDisabledStatus(row.status), onClick: () => openToggle(row) },
          ]"
        />
      </template>
    </BasicTable>
      ...
      <div v-if="detailModal.data.value" class="space-y-3 text-sm">
        <div class="grid grid-cols-2 gap-x-6 gap-y-3">
          ...
          <div>
            ...
            <Badge :variant="APPLY_STATUS_VARIANT[detailModal.data.value.status]">{{ APPLY_STATUS_LABEL[detailModal.data.value.status] ?? detailModal.data.value.statusLabel }}</Badge>
          </div>
        ...
        <div v-if="isPendingStatus(detailModal.data.value.status)" v-auth="'b2b:store:review'" class="flex justify-end gap-2 pt-4 border-t border-border">
        ...
        <div v-if="isApprovedStatus(detailModal.data.value.status) || isDisabledStatus(detailModal.data.value.status)" v-auth="'b2b:store:review'" class="flex justify-end gap-2 pt-4 border-t border-border">
    ...
    <BasicModal
      ...
      :title="toggleModal.data.value?.status === 1 ? '停用门店' : '启用门店'"
      :description="`确认${toggleModal.data.value?.status === 1 ? '停用' : '启用'} ${toggleModal.data.value?.storeName || ''}？`"
